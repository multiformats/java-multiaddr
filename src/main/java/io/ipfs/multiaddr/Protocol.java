  ...
  public byte[] addressToBytes(String addr) {
    try {
      switch (type) {
        ...
        case IP6ZONE:
          if (addr.isEmpty()) throw new IllegalStateException("Empty IPv6 zone!");
          if (addr.contains("/")) throw new IllegalStateException("IPv6 zone ID contains '/'");
        ...
        case SCTP:
          ...
          if (x > 65535)
            throw new IllegalStateException(
                "Failed to parse " + type.name + " address " + addr + " (> 65535");
          return new byte[] {(byte) (x >> 8), (byte) x};
        case P2P:
        case IPFS:
          {
            Multihash hash;
            if (addr.startsWith("Qm") || addr.startsWith("1")) hash = Multihash.fromBase58(addr);
            else {
              Cid cid = Cid.decode(addr);
              if (cid.codec != Cid.Codec.Libp2pKey)
                throw new IllegalStateException(
                    "failed to parse p2p addr: " + addr + " has the invalid codec " + cid.codec);
        ...
        case GARLIC32:
          {
            ...
            if (addr.length() < 55 && addr.length() != 52 || addr.contains(":")) {
              throw new IllegalStateException(
                  String.format(
                      "Invalid garlic addr: %s not a i2p base32 address. len: %d",
                      addr, addr.length()));
            }
        ...
        case GARLIC64:
          {
            ...
            if (addr.length() < 516 || addr.length() > 616 || addr.contains(":")) {
              throw new IllegalStateException(
                  String.format(
                      "Invalid garlic addr: %s not a i2p base64 address. len: %d",
                      addr, addr.length()));
            }
            ...
            byte[] hashBytes =
                Multibase.decode(
                    Multibase.Base.Base64.prefix + addr.replaceAll("-", "+").replaceAll("~", "/"));
            byte[] varint = new byte[(32 - Integer.numberOfLeadingZeros(hashBytes.length) + 6) / 7];
            putUvarint(varint, hashBytes.length);
            bout.write(varint);
            bout.write(hashBytes);
