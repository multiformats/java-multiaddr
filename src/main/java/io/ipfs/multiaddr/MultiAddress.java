...
public class MultiAddress {
  ...
  public boolean isPublic(boolean testReachable) {
    ...
    try {
      for (int i = 0; i < parts.length; i++) {
        if (parts[i].equals(Protocol.Type.IP6ZONE.name)) return true;
        if (parts[i].equals(Protocol.Type.IP4.name) || parts[i].equals(Protocol.Type.IP6.name)) {
          ...
          if (ip.isLoopbackAddress()
              ...
              || ip.isAnyLocalAddress()) return false;
  ...
  public boolean isTCPIP() {
    ...
    if (!parts[0].startsWith("ip")) return false;
    if (!parts[2].equals("tcp")) return false;
    return true;
  }

  public String getHost() {
    String[] parts = toString().substring(1).split("/");
    if (parts[0].startsWith("ip") || parts[0].startsWith("dns")) return parts[1];
    throw new IllegalStateException("This multiaddress doesn't have a host: " + toString());
  }

  public int getPort() {
    String[] parts = toString().substring(1).split("/");
    if (parts[2].startsWith("tcp") || parts[2].startsWith("udp")) return Integer.parseInt(parts[3]);
  ...
  private static byte[] decodeFromString(String addr) {
    ...
    if (parts[0].length() != 0) throw new IllegalStateException("MultiAddress must start with a /");
    ...
    try {
      for (int i = 1; i < parts.length; ) {
        ...
        String component =
            p.isTerminal()
                ? Stream.of(Arrays.copyOfRange(parts, i, parts.length))
                    .reduce("", (a, b) -> a + "/" + b)
                : parts[i++];
  ...
  public boolean equals(Object other) {
    if (!(other instanceof MultiAddress)) return false;
